...
/***/ function(module, exports) {
	...
	$(document).ready(function () {
	  ...
	  * */
		function setCoordinate(originData, startX, startY, widthMargin, canvasHeight) {
	  ...
	  * */
		function drawTrapezoid(ctx, initWidth, initHeight, schoolList, trapezoidStyle, schoolNumNameStyle, lineDotStyle, title, trapezoidGap, contextFontStyle) {
			...
			for (var i = 0, len = schoolList.length; i < len; i++) {
				...
				ctx.beginPath();
				ctx.arc(startX + trapezoidWidthDown * (250 / 286), startY + initHeight / 2, 4, 0, 2 * Math.PI, true);
		...
		drawCircle(context, centerX, centerY, '#ffffff', '#e4e4e4', 10, radius, 1);
		drawCircle(context, centerX, centerY, '#ffffff', '#f9be00', 12, radius, 0.4, 'round');
		context.textBaseline = 'middle';
		context.textAlign = "center";

		drawCircleText(context, 'normal 40pt Calibri', '#f9be00', '40', centerX - 10, centerY - 16);
		drawCircleText(context, 'normal 20pt Calibri', '#f9be00', '%', centerX + 35, centerY - 10);
		drawCircleText(context, 'normal 18px Calibri', '#b6b6b6', '录取概率', centerX, centerY + 22);
	  ...
	  **/
		...
		setCoordinate(originData, startX, startY, widthMargin, 400);

		offsetY = lowestPercent < 0.01 ? 80 : lowestPercent < 0.1 ? 50 : 30;

		drawCoordinate(context, coordData, yearColor, historyColor, currentColor, 20, labelWitth, canvas.width, canvas.height, offsetY);
		drawLabel(context, coordData, 36, 8, 20, canvas.height, offsetY, labelWitth, canvas.width);
		...
		drawTrapezoid(context, width, height, schoolData, trapezoidStyle, schoolNumNameStyle, lineDotStyle, "（考生数量）", 6, contextFontStyle);
	});
