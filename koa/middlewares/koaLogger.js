...
function getKoaLogger(logger4js, options) {
    ...
    return function*(next) {
        ...
        if (thislogger.isLevelEnabled(level) || options.level === 'auto') {
            ...
            var isResourceReq = url.match(/\.js$|\.css$|\.ico$|.\html$|\.png$/);
            ...
            if (thislogger.isLevelEnabled(level)) {
                ...
                if (typeof fmt === 'function') {
                    ...
                    if (!isResourceReq) {
                        console.log(' \x1B[32m %s\x1B[0m', format(fmt, combined_tokens));
                    }
                }
            }
        }
...
function assemble_tokens(ctx, custom_tokens) {
    ...
    default_tokens.push({ token: ':user-agent', replacement:  ctx.userBrowser || ctx.headers['user-agent'] });
    ...
    ctx.response.length || '-' });
    default_tokens.push({ token: ':user-id', replacement: (ctx.session.user && ctx.session.user['_id']) || ''});
