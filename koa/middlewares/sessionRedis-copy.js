...
module.exports = function (opts) {
  ...
  return function *(next) {
    ...
    sid = this.sessionId = this.cookies.get(keySessionId, cookieOption);
    uid = this.uid = this.cookies.get(keyUId, cookieOption);

    if (uid) {
      try {
        json = yield client.get(uid); // 根据sid取出session数据
      }catch (e) {
        debug('encounter error %s', e);
        json = null;
      }
    } else{
      sid = this.sessionId = sessionIdGenerator.generateUid();
      uid = this.uid = sessionIdGenerator.generateUid();
    }
    ...
    try {
      ...
      sid = this.session.sessionId = this.sessionId;
